...
-record(state, { client = undefined :: undefined | binary(),
                                                  ...
                                                  <<"ENTERROOM">> ->
                                                            ...
                                                            case length(Participants) of 
                                                                1 ->
                                                                    ...
                                                                    {ok, Req, S, hibernate};
                                                                _ ->
                                                                    R = iolist_to_binary(jsonerl:encode({{type,<<"WRONGROOM">>}})), {reply, {text,<<R/binary>>}, Req, StateNew, hibernate}
                                                                    end;
                                                  _ ->
...
reply2peer(R, Room) ->
    [P ! <<R/binary>> || P <-
    gproc:lookup_pids({p,l,Room}) -- [self()]].
    
generate_room() ->
    rand:seed(so:system_time()),
    rand:uniform(999999).
